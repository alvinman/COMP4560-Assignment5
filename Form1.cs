...
namespace asgn5v1
{
	...
	public class Transformer : System.Windows.Forms.Form
	{
        ...
        public Transformer()
		{
			...
			Text = "COMP 4560:  Assignment 5 (200830) Alvin Man";
			ResizeRedraw = true;
        ...
        private double[,] shapeInitializationMatrix()
        {
            double[,] scaleMatrix = new double[4, 4];
            double[,] translationMatrixA = new double[4, 4];
            double[,] reflectionMatrix = new double[4, 4];
            double[,] translationMatrixB = new double[4, 4];

            //get screen coordinates
            double screenHeight = this.ClientSize.Height;
            double screenWidth = this.ClientSize.Width;

            //loop thru points to find the highest point and lowest point, this will give us the height
            double highestPoint = 0;
            double lowestPoint = screenHeight;
            int yCoord = 1;
            
            for (int i = 0; i < numpts; i++)
            {
                if (vertices[i, yCoord] > highestPoint)
                {
                    highestPoint = vertices[i, yCoord];
                }

                if (vertices[i, yCoord] < lowestPoint)
                {
                    lowestPoint = vertices[i, yCoord];
                }
            }

            double initialShapeHeight = highestPoint - lowestPoint;

            //scale it so that it makes the height = 1/2 screen height
            double differenceFactor = screenHeight / initialShapeHeight;
            double scaleFactor = differenceFactor / 2;

            scaleMatrix = getScaleMatrix(scaleFactor, scaleFactor, scaleFactor);

            //move the center vertex to the origin (assuming vertex 0 is center for now)
            double xTranslation = -vertices[0, 0];
            double yTranslation = -vertices[0, 1];
            double zTranslation = -vertices[0, 2];

            translationMatrixA = getTranslationMatrix(xTranslation, yTranslation, zTranslation);

            //reflection over x-axis
            reflectionMatrix = getReflectionMatrix("x");

            //translate to the center of the screen
            translationMatrixB = getTranslationMatrix(screenWidth/2, screenHeight/2, 0);

            //multiply all matrices together for initial transformation matrix
            double[,] temp1 = multiply4x4Matrix(translationMatrixA, scaleMatrix);
            double[,] temp2 = multiply4x4Matrix(temp1, reflectionMatrix);
            return multiply4x4Matrix(temp2, translationMatrixB);
        }

        private double[,] multiply4x4Matrix(double[,] matrixA, double[,] matrixB)
        {
        ...
        private double[,] getScaleMatrix(double xScaleFactor, double yScaleFactor, double zScaleFactor)
        {
            double[,] result = new double[4, 4];
            setIdentity(result, 4, 4);
            result[0, 0] = xScaleFactor;
            result[1, 1] = yScaleFactor;
            result[2, 2] = zScaleFactor;

            return result;            
        }

        private double[,] getTranslationMatrix(double xTranslation, double yTranslation, double zTranslation)
        {
            double[,] result = new double[4, 4];
            setIdentity(result, 4, 4);
            result[3, 0] = xTranslation;
            result[3, 1] = yTranslation;
            result[3, 2] = zTranslation;

            return result;
        }

        private double[,] getReflectionMatrix(string overAxis)
        {
            double[,] result = new double[4, 4];
            setIdentity(result, 4, 4);
            if (overAxis.Equals("y"))
            {
                result[0, 0] = -1;
            }
            if (overAxis.Equals("x"))
            {
                result[1, 1] = -1;
            }
            return result;
